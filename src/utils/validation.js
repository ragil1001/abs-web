...
export const validationRules = {
  ...
  number: (message = 'Harus berupa angka') => (value) => {
    if (!value) return true;
    if (isNaN(value)) {
      return message;
    }
  ...
  min: (minValue, message = null) => (value) => {
    if (!value) return true;
  ...
  max: (maxValue, message = null) => (value) => {
    if (!value) return true;
    const num = parseFloat(value);
  ...
  confirmPassword: (passwordField, message = 'Password tidak sama') => (value, data) => {
    if (!value) return true;
  ...
  date: (message = 'Format tanggal tidak valid') => (value) => {
    if (!value) return true;
  ...
  pastDate: (message = 'Tanggal harus di masa lalu') => (value) => {
    if (!value) return true;
    const date = new Date(value);
  ...
  futureDate: (message = 'Tanggal harus di masa depan') => (value) => {
    if (!value) return true;
    const date = new Date(value);
    const today = new Date();
  ...
  beforeOrEqualToday: (message = 'Tanggal tidak boleh di masa depan') => (value) => {
    if (!value) return true;
    const date = new Date(value);
    const today = new Date();
    today.setHours(23, 59, 59, 999);
  ...
  afterField: (fieldName, message = null) => (value, data) => {
  ...
  nik: (message = 'NIK harus 16 digit angka') => (value) => {
    if (!value) return true;
    if (!/^\d{16}$/.test(value)) {
      return message;
    }
  ...
  custom: (validator, message = 'Validasi gagal') => (value, data) => {
    const result = validator(value, data);
...
export const loginValidator = new Validator()
  ...
  .addRule('password', [
    ...
    validationRules.minLength(VALIDATION.MIN_PASSWORD_LENGTH, `Password minimal ${VALIDATION.MIN_PASSWORD_LENGTH} karakter`)
  ]);
...
export const karyawanValidator = new Validator()
  ...
  .addRule('nama', [
    ...
    validationRules.maxLength(VALIDATION.MAX_NAME_LENGTH, `Nama maksimal ${VALIDATION.MAX_NAME_LENGTH} karakter`)
  ])
  .addRule('divisi_id', [
    validationRules.required('Divisi wajib dipilih')
  ])
  .addRule('jabatan_id', [
    validationRules.required('Jabatan wajib dipilih')
  ])
  ...
  .addRule('tempat_lahir', [
    ...
    validationRules.maxLength(VALIDATION.MAX_NAME_LENGTH, `Tempat lahir maksimal ${VALIDATION.MAX_NAME_LENGTH} karakter`)
  ])
  ...
  .addRule('tanggal_bergabung', [
    ...
    validationRules.date(),
    validationRules.beforeOrEqualToday('Tanggal bergabung tidak boleh di masa depan')
  ])
