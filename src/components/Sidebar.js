...
const Sidebar = memo(
  ({ collapsed, isMobile, currentPage, onNavigate, onClose }) => {
          ...
          submenu: [
            {
              ...
              icon: UserPlus,
              page: "assign-karyawan",
            },
            {
              id: "jadwal",
              label: "Jadwal Karyawan",
              icon: Calendar,
              page: "jadwal-karyawan",
            },
            {
              id: "tukar-shift",
              label: "Info Tukar Shift",
              icon: RefreshCcw,
              page: "tukar-shift",
            },
            {
    ...
    useEffect(() => {
      if (!collapsed) {
        const parentMenu = menuItems.find(
          (item) =>
            item.submenu &&
            item.submenu.some((subitem) => subitem.page === currentPage)
        );
        if (parentMenu) {
                ...
                <div key={item.id} className="relative">
                  ...
                  {item.submenu && collapsed && (
                    <div className="absolute left-full top-0 ml-2 w-56 bg-white border border-gray-200 rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-[100]">
                      <div className="p-3">
                        <div className="text-xs font-semibold text-gray-900 mb-3 px-2">
                          {item.label}
                        </div>
