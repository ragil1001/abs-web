...
import {
  Search, Users, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, 
  MapPin, Loader2
} from "lucide-react";
...
const AssignPage = ({ onNavigateToDetail }) => {
  ...
  const fetchProjectStats = useCallback(async (projectList) => {
    const stats = {};
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 mb-6">
        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-4">
          <div className="flex-1">
            ...
            <p className="text-gray-600">Kelola penugasan karyawan untuk setiap project</p>
          </div>
        </div>
      </div>
              ...
              {fetchLoading && paginationData.items.length === 0 ? (
                ...
                paginationData.items.map((p, idx) => (
                  <tr key={p.id} className={`border-b border-gray-100 hover:bg-orange-50 transition-colors ${idx % 2 === 0 ? "bg-white" : "bg-gray-50"}`}>
                      ...
                      <div className="space-y-1">
                        {formatShifts(p.shifts).map((shift, idx) => (
                          <div key={idx} className="text-xs">
                            ...
                            <span className="text-gray-600 ml-1">{shift.time}</span>
                          </div>
                        ))}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div>
                        <div className="font-medium text-gray-900 text-sm">{p.lokasi_nama || p.lokasi?.nama}</div>
                        <button
                        ...
                        >
                          ...
                          {p.lokasi_latitude || p.lokasi?.latitude}, {p.lokasi_longitude || p.lokasi?.longitude}
                        </button>
                      </div>
                    </td>
