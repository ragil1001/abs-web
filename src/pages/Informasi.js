    ...
    const fetchKaryawan = async () => {
      ...
      try {
        ...
        if (projectFilter !== "all") {
          if (jabatanFilter !== "all") {
            data = data.filter(
              (k) => k.jabatan?.id === parseInt(jabatanFilter)
            );
          }
          if (searchTerm.trim()) {
    ...
    <div className="space-y-3">
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
        <p className="text-sm text-blue-800">
          ...
          Pilih minimal satu filter (Jabatan, Project, atau Search).
        </p>
      </div>
          ...
          <div className="flex items-center gap-2">
            {filteredKaryawan.length > 0 && (
              <>
                ...
                >
                  {selectedIds.length === filteredKaryawan.length &&
                  filteredKaryawan.length > 0 ? (
                    <>
        ...
        {!hasAppliedFilter ? (
          <div className="p-8 text-center text-gray-500">
            ...
            <p className="font-medium">
              Gunakan Filter untuk Menampilkan Karyawan
            </p>
            <p className="text-sm mt-1">
              Pilih Jabatan, Project, atau cari berdasarkan Nama/NIK
            </p>
          </div>
        ) : loading ? (
...
const Informasi = () => {
  ...
  const processedData = useMemo(() => {
    ...
    if (targetTypeFilter !== "all") {
      filtered = filtered.filter(
        (info) => info.target_type === targetTypeFilter
      );
    }
  ...
  const handleOpenPenerimaModal = useCallback(
    async (info) => {
      setSelectedInformasi(info);
      setPenerimaPage(1);
  ...
  const handleSubmit = async () => {
    ...
    const result = await Swal.fire({
      ...
      html: `${
        selectedInformasi ? "Update" : "Simpan"
      } informasi sebagai draft?`,
      icon: "question",
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-${
                          TARGET_TYPES[info.target_type]?.color
                        }-100 text-${
                          TARGET_TYPES[info.target_type]?.color
                        }-700`}
                      >
        ...
        <div className="px-6 py-4 border-t flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {Array.from(
              { length: Math.min(paginationData.pagination.last_page, 5) },
              (_, i) => {
                let pageNum;
                if (paginationData.pagination.last_page <= 5) {
                  ...
                  currentPage >=
                  paginationData.pagination.last_page - 2
                ) {
            ...
            <button
              onClick={() =>
                setCurrentPage(
                  Math.min(paginationData.pagination.last_page, currentPage + 1)
                )
              }
                      ...
                      <div className="border rounded-lg max-h-60 overflow-y-auto">
                        {targetOptions.length === 0 ? (
                          ...
                          targetOptions.map((option, idx) => {
                            ...
                            const isChecked =
                              currentIds.includes(optionValueInt);
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4">
              <div>
                ...
                <div className="flex items-center gap-4 text-sm text-gray-600">
                  <span>
                    Dibuat: {formatDate(selectedInformasi.created_at)}
                  </span>
                  {selectedInformasi.dikirim_at && (
                    <span>
                      • Dikirim: {formatDate(selectedInformasi.dikirim_at)}
                    </span>
                  )}
                </div>
              </div>
              ...
              <div className="flex gap-2">
                <span
                  className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-${
                    TARGET_TYPES[selectedInformasi.target_type]?.color
                  }-100 text-${
                    TARGET_TYPES[selectedInformasi.target_type]?.color
                  }-700`}
                >
              ...
              <div className="border-t pt-4">
                ...
                {selectedInformasi.file_url ? (
                  <button
                    onClick={() =>
                      window.open(selectedInformasi.file_url, "_blank")
                    }
                  ...
                  <div className="flex items-start gap-3 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                        Informasi ini tidak memiliki file lampiran atau file
              ...
              <div className="border-t pt-4">
                ...
                <p className="text-gray-700">
                  {selectedInformasi.target_names}
                </p>
              </div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    <div className="bg-blue-50 rounded-lg p-3">
                      <div className="text-2xl font-bold text-blue-600">
                        {selectedInformasi.total_penerima}
                      </div>
                      <div className="text-sm text-gray-600">
                        Total Penerima
                      </div>
                    ...
                    <div className="bg-green-50 rounded-lg p-3">
                      <div className="text-2xl font-bold text-green-600">
                        {selectedInformasi.total_dibaca}
                      </div>
            ...
            <div className="px-6 py-4 border-t flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() =>
                    setPenerimaPage(
                      Math.min(penerimaTotalPages, penerimaPage + 1)
                    )
                  }
