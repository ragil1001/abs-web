...
const PengajuanLembur = ({ navigationDetail = null }) => {
    ...
    if (
      ...
      setTimeout(async () => {
        ...
        if (submission) {
          ...
          try {
            const result = await pengajuanLemburAPI.getById(
              navigationDetail.id
            );
            if (result.success) {
  ...
  const handleSubmitConfirmation = async () => {
    ...
    try {
      const result = await pengajuanLemburAPI.prosesPengajuan(
              ...
              <tbody>
                {submissions.map((submission, index) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      {getKodeHariBadge(submission.kode_hari)}
                    </td>
                    <td className="px-4 py-3">
              ...
              <div className="bg-gray-50 rounded-xl p-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="font-semibold text-gray-900">
                      {formatDate(selectedSubmission.tanggal)}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    {selectedSubmission.file_skl_url ? (
                      <button
                        onClick={() =>
                          window.open(selectedSubmission.file_skl_url, "_blank")
                        }
                      ...
                      <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg">
                        ...
                        <div>
                          <p className="text-sm font-medium text-amber-900">
                            File SKL Tidak Tersedia
                          </p>
                          <p className="text-sm text-amber-700 mt-1">
                            File Surat Keterangan Lembur untuk pengajuan ini
                            sudah tidak tersedia atau telah dihapus dari sistem.
                          </p>
                        </div>
                      </div>
                    )}
