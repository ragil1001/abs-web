...
const DataJabatan = () => {
  ...
  const processedData = useMemo(() => {
    ...
    if (searchTerm.trim()) {
      ...
      filtered = allJabatans.filter(jabatan =>
  ...
  const handleOpenEditModal = useCallback((jabatan) => {
    resetForm();
  ...
  const handleSubmit = async () => {
    ...
    const message = selectedJabatan 
      ? `Apakah Anda yakin ingin memperbarui jabatan <b>${selectedJabatan.nama}</b> menjadi <b>${formData.nama.trim()}</b>?`
        ...
        try {
          ...
          rows.forEach((row, index) => {
            ...
            if (existingNames.includes(namaLower)) {
              errors.push(`Baris ${rowNumber}: Jabatan "${namaClean}" sudah ada`);
              return;
            }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (result.isConfirmed) {
        ...
        clearApiCache();
        toast.success(`${validData.length} jabatan berhasil diimport`, { autoClose: 5000 });
        await fetchAllData();
        handleCloseModal();
      }
    } catch (err) {
  ...
  const exportToExcel = async () => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/jabatans/export`, {
        method: 'GET',
        headers: {
  ...
  const downloadTemplate = () => {
    try {
      const ws = XLSX.utils.aoa_to_sheet([["Nama Jabatan"], ["Software Engineer"], ["HR Manager"], ["Finance Analyst"]]);
      ...
      <div className="p-6 bg-gray-50 min-h-screen space-y-8">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 animate-pulse space-y-4">
          ...
          <div className="flex gap-2">
            {[1, 2, 3].map(i => <div key={i} className="h-10 bg-gray-200 rounded flex-1"></div>)}
          </div>
        </div>
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Data Jabatan</h1>
          <p className="text-gray-600">Kelola daftar jabatan perusahaan</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setShowImportModal(true)} className="px-4 py-2 bg-blue-600 text-white rounded-lg flex items-center gap-2 hover:bg-blue-700">
            <Upload className="w-4 h-4" /> Import
          </button>
          <button onClick={exportToExcel} className="px-4 py-2 bg-green-600 text-white rounded-lg flex items-center gap-2 hover:bg-green-700">
            <Download className="w-4 h-4" /> Export
          </button>
          <button onClick={handleOpenAddModal} className="px-4 py-2 bg-orange-600 text-white rounded-lg flex items-center gap-2 hover:bg-orange-700">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
          <div className="flex items-center gap-4">
            ...
            <select value={itemsPerPage} onChange={(e) => handleItemsPerPageChange(parseInt(e.target.value))} className="px-3 py-1 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500">
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gradient-to-r from-orange-500 to-orange-600 text-white">
              <tr>
                {[{ key: "id", label: "ID" }, { key: "nama", label: "Nama Jabatan" }].map((col) => (
                  <th key={col.key} onClick={() => handleSort(col.key)} className="px-6 py-4 text-left font-semibold cursor-pointer hover:bg-orange-600">
              ...
              {paginationData.items.map((j) => (
                <tr key={j.id} className="border-b hover:bg-orange-50">
                  ...
                  <td className="px-6 py-4 text-center">
                    <button onClick={() => handleOpenEditModal(j)} className="p-1.5 text-blue-600 hover:bg-blue-100 rounded-lg">
                      <Edit className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDelete(j)} className="p-1.5 text-red-600 hover:bg-red-100 rounded-lg ml-2">
        ...
        <div className="fixed inset-0 bg-gray-900/50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="px-6 py-4 border-b flex justify-between items-center">
              ...
              <button onClick={handleCloseModal} className="p-2"><X className="w-5 h-5" /></button>
            </div>
            <div className="px-6 py-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">Nama Jabatan *</label>
              <input type="text" value={formData.nama} onChange={(e) => setFormData({ ...formData, nama: e.target.value })} placeholder="Masukkan nama jabatan" className={`w-full border px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 ${formErrors.nama ? "border-red-500" : "border-gray-200"}`} disabled={submitLoading} maxLength={255} />
              {formErrors.nama && <p className="text-red-500 text-sm mt-1">{formErrors.nama[0]}</p>}
            </div>
            <div className="px-6 py-4 border-t flex justify-end gap-2">
              <button onClick={handleCloseModal} disabled={submitLoading} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg">Batal</button>
              <button onClick={handleSubmit} disabled={submitLoading} className="px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 disabled:opacity-50">
        ...
        <div className="fixed inset-0 bg-gray-900/50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg">
            <div className="px-6 py-4 border-b flex justify-between items-center">
              ...
              <button onClick={() => setShowImportModal(false)} disabled={importLoading} className="p-2"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6">
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
                <Upload className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                <h3 className="text-lg font-medium text-gray-900 mb-2">{importFile ? importFile.name : "Pilih file Excel"}</h3>
                <p className="text-gray-600 mb-4">Format: .xlsx, .xls (Maksimal 2MB)</p>
                <input type="file" accept=".xlsx,.xls" className="hidden" id="jabatan-import" onChange={handleFileSelect} disabled={importLoading} />
                <label htmlFor="jabatan-import" className={`px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors cursor-pointer inline-block ${importLoading ? 'opacity-50 cursor-not-allowed' : ''}`}>
              ...
              <div className="mt-4 flex gap-2">
                <button onClick={downloadTemplate} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700" disabled={importLoading}>
                  Download Template
                </button>
                <button onClick={handleImport} className={`flex-1 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed ${!importFile ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!importFile || importLoading}>
                  {importLoading ? "Mengimport..." : "Import Data"}
                </button>
              </div>
            </div>
            <div className="px-6 py-4 border-t flex justify-end">
              <button onClick={() => setShowImportModal(false)} disabled={importLoading} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg">
                Tutup
              </button>
            </div>
          </div>
